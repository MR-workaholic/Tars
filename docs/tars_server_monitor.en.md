...
For all business services based on the Tars framework implementation, Tars Node can perform real-time monitoring. The general service exits abnormally, and the phenomenon of zombie is actively monitored by the Tars Node, which makes the service run more reliable.

# 2. Service monitoring principle

## 2.1. Overview of service monitoring principles

The Tars Node service will open a monitoring thread to be responsible for timing (interval time can be configured) to poll the monitoring of the running status of all services managed by the Node, and to report the running status of the service to the master. If a service meets the relevant trigger conditions, Tars Node takes corresponding measures to manage the service, so that the running status of the service is in real time under the monitoring of Node.

The above monitoring thread mainly includes the following key features:

1.Frequency of monitoring thread polling service (in seconds)

2.Timeout period (in seconds) for business service heartbeat reporting. If the business service processing under a node is time consuming, the user can consider increasing this value.

3.The frequency (in seconds) that the Tars Node reports the running status of the service to the master.

4.Tars Node has two ways to report the running status of the service to the master: single report and batch report. The batch report mode is selected by default. If the Tars Node manages a large number of services, you can choose the batch method to improve efficiency.
...
### 2.2.2. A strategy for judging service heartbeat timeout

The heartbeat timeout period of the service is configured in the Node. This time limit applies to the heartbeat timeout period of all services managed by the Node.
...
### 2.2.3. A strategy for judging service is allowed to start
...
3.Some services may indeed have systemic problems that prevent them from starting successfully. In order to prevent frequent restarts of these services, the Node monitor adopts the policy of ¡°service restart penalty time¡±: that is, each time the service is restarted, the number of restarts and the restart time point will be recorded, and the maximum restart time is N times in X seconds. After N times, the service still fails to start, and then try to restart the service at a frequency of Y seconds.
